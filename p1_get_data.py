...
def subsample_by_timegap(df, id_col, time_col, timegap_hours=2):
    """
...
def main(file_in, output_dir, log_dir):
    ...
    with sb.PrintRedirector(log_dir / f'{file_in.stem}.log'):
        ...
        # keep only data within main extent
        df = df[
            (df['lon'] >= main_extent[0]) & (df['lon'] <= main_extent[1]) &
            (df['lat'] >= main_extent[2]) & (df['lat'] <= main_extent[3])
        ]
